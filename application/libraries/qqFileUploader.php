...
class qqFileUploader {
    ...
    function __construct($params = array()){
        ...
        if (isset($params['webservice']) && isset($_FILES['qqfile'])) {
    ...
    function handleUpload(){
        ...
        if ($is){
            if(! isset($data['error'])){
				die("{'error':'No hay data error'}");
                return array('success'=>true,'filename'=>$data);
            }else{
				die("{'error':'Hay data error'}");
...
class SaveFtp
{
	...
	public function __Construct($params = array())
	{
		...
		{
			 die("{'error':'Se debe definir el Modelo,Metodo y Parametros'}");
		}
	}
